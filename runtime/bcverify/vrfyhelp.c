	...
	*argCount = 0;
	...
	if ((!(romMethod->modifiers & J9AccStatic)) && (argMax > 0)) {	
		...
		if ((J9UTF8_DATA(utf8string)[0] == '<')	&& (J9UTF8_DATA(utf8string)[1] == 'i') && (classIndex != BCV_JAVA_LANG_OBJECT_INDEX)) {
			...
			if (J9ROMCLASS_IS_PRIMITIVE_VALUE_TYPE(romClass)) { /* This is temporary and will have to be changed when <vnew> is introduced (along with all other code that references J9VM_OPT_VALHALLA_NEW_FACTORY_METHOD) */
	...
	for (i = 1; args[i] != ')'; i++) {
		...
		if (IS_REF_OR_VAL_SIGNATURE(args[i])) {
			U_8 *string;
			U_16 length = 0;
			UDATA type = IS_QTYPE(args[i]) ? BCV_PRIMITIVE_VALUETYPE : BCV_GENERIC_OBJECT;
...
pushClassType(J9BytecodeVerificationData * verifyData, J9UTF8 * utf8string, UDATA * stackTop)
{
	...
	if (firstChar == '[') {
		...
		UDATA type = IS_QTYPE(firstChar) ? BCV_PRIMITIVE_VALUETYPE : BCV_OBJECT_OR_ARRAY;
...
{ 
	...
	*reasonCode = 0;
	...
	if (*targetClassName == '[') {
		...
		UDATA type = IS_QTYPE(*targetClassName) ? BCV_PRIMITIVE_VALUETYPE : BCV_OBJECT_OR_ARRAY;
