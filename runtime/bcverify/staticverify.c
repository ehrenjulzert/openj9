...
*/
		...
		case CFR_BC_new:
			...
			if ((CFR_BC_new == bc) && bcvIsReferenceTypeDescriptor(info)) {
				errorType = NEW_INVALID_REFERENCE_TYPE_DESCRIPTOR__ID;
				goto _verifyError;
			}
			if ((CFR_BC_aconst_init == bc) && ('[' == *info->bytes)) {
				errorType = ACONST_INIT_INVALID_ARRAY__ID;
				goto _verifyError;
			}
#endif /* #if defined(J9VM_OPT_VALHALLA_VALUE_TYPES) */
			break;
...
#if !defined(J9VM_OPT_VALHALLA_VALUE_TYPES) /* This #if !defined check should be removed (along with all other code that references J9VM_OPT_VALHALLA_NEW_FACTORY_METHOD) once the <vnew> method is introduced */
				} else {
